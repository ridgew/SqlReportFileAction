...
namespace SqlReportFileAction
{
    public class SqlReportFile : IDisposable
    {
        ...
        public void BuildSplitFileWithRows(List<ReportRow> fileRows, string savePath)
        {
            ...
            {
                int crtLine = 0;
                while (crtLine < ReportFile_HeadLine_Length)
                {
                    crtLine++;
                    lineBuilder.AppendLine(sr.ReadLine());
                }
            }
            #endregion
            ...
            {
                sw.Write(lineBuilder.ToString());

                string fieldStr = string.Empty;
                for (int i = 0, j = fileRows.Count; i < j; i++)
                {
                    lineBuilder.Clear();

                    ReportRow row = fileRows[i];
                    for (int m = 0, n = Columns.Count; m < n; m++)
                    {
                        var col = Columns[m];
                        if (m < n - 1)
                        {
                            fieldStr = row[col.ColumnName].PadRight(col.ColumnLength, ' ');
                            lineBuilder.Append(fieldStr);
                            lineBuilder.Append(new string(' ', ReportFile_Space_Length));
                        }
                        else
                        {
                            fieldStr = row[col.ColumnName];
                            lineBuilder.Append(fieldStr);
                        }
                    }
